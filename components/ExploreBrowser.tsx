...
export default function ExploreBrowser({
  ...
  /** When true, sync the selected mood with the `?mood=` query param (used by /explore). */
  readUrlParam?: boolean;
  /** Bottom SEO block. Falls back to the generic mindfulness copy when omitted. */
  children?: React.ReactNode;
}) {
  const searchParams = useSearchParams();
  const moodParam = readUrlParam ? (searchParams.get("mood") as Mood | null) : null;
  const startMood =
    moodParam && allMoods.includes(moodParam) ? moodParam : initialMood;

  const [activeMood, setActiveMood] = useState<Mood | null>(startMood);
  const [activeStyle, setActiveStyle] = useState<Style | null>(null);

  useEffect(() => {
    if (readUrlParam && moodParam && allMoods.includes(moodParam)) {
      setActiveMood(moodParam);
    }
  }, [moodParam, readUrlParam]);
